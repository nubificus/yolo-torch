...
float letterbox(cv::Mat &input_image, cv::Mat &output_image, const std::vector<int> &target_size) {
	...
	cv::copyMakeBorder(output_image, output_image, top, bottom,
			   left, right, cv::BORDER_CONSTANT, cv::Scalar(114.));
	return resize_scale;
}
...
int main(int argc, char **argv) {
	...
	try {
		...
		if (vaccel_resource_init(&vmodel, argv[1], VACCEL_RESOURCE_MODEL)) {
			vaccel_warn("Could not initialize resource");
			goto sess_release;
		}
	
		if (vaccel_resource_register(&vmodel, &sess)) {
			fprintf (stderr, "Could not register model resource\n");
			goto model_release;
		}
		...
		v_image->data = reinterpret_cast<void*>(image_tensor.data_ptr<float>());
		v_image->size = 3 * 640 * 640 * sizeof(float);

		v_out = (struct vaccel_torch_tensor*) malloc (sizeof (struct vaccel_torch_tensor));
		if (!v_out) {
			vaccel_warn("Could not initialize output tensor");
			goto img_destroy;
		}
...
model_unreg:
	if (vaccel_resource_unregister(&vmodel, &sess))
		vaccel_warn("Could not unregister model resource");
model_release:
	if (vaccel_resource_release(&vmodel))
		vaccel_warn("Can't release resource");
sess_release:
